...
class RF95:
    ...
    def waitPacketSent(self):
        return radiohead.waitPacketSent()

    def waitAvailableTimeout(self, timeout):
        b = radiohead.waitAvailableTimeout(timeout)
        return b
        if (b == 1):
            return True
        else:
    ...
    def recv2(self):
        r = radiohead.recv(buf, l)
        return (ffi.string(buf), l[0], r)
    ...
    def setModeTx(self):
        return radiohead.setModeTx()
    
    def setModeRx(self):
        return radiohead.setModeRx()
